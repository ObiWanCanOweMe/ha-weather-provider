...
| Weather Conditions Card | Good showcase for TWC enrichment. Pollen forecast sensors are available when enabled; air quality and pollen observation counts remain separate follow-up work. |
...
| Pollen | `docs/twc_api/API - Standard - Lifestyle Indices - Pollen Index - v2.0.pdf`, `docs/twc_api/API - Standard US Pollen Observation v1.0.pdf`, `docs/twc_api/API - Standard - Pollen Historical - v3.pdf` | First slice: optional forecast sensors for grass/tree/ragweed index and category. Deferred: U.S.-only total/tree/grass/weed/mold observation counts and descriptions. |
...
| Tropical Weather | `docs/twc_api/API - Standard - Tropical - *.pdf`, `docs/twc_api/API - Standard - v3 - Tropical Models - *.pdf` | Optional storm summary sensors/entities for active storms, current position, forecast cone/path, model detail, and raw bulletins where values map cleanly to Home Assistant. |
